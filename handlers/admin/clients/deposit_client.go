...
func AdminClientDeposit(db *gorm.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = db.Transaction(func(tx *gorm.DB) error {
			err := utils.ClientAddBalance(tx, req.ClientID, req.Amount)
			if err != nil {
				return err
			}
			err = utils.ClientLogTransaction(tx, req.ClientID, req.Amount, "deposit")
			if err != nil {
				return err
			}
			err = utils.AdminLogTransaction(tx, req.ClientID, req.Amount, "withdraw")
			if err != nil {
				return err
			}
